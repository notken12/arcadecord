...
async function action_guess(game, action) {
    ...
    if (game.data.guesses[0].length === game.data.guesses[1].length) {
        if (game.data.guesses[0].length >= MAX_GUESSES && game.data.guesses[1].length >= MAX_GUESSES) {
            ...
            let player0Right = player0Guesses[player0Guesses.length - 1].word === game.data.answers[1];

            let player1Guesses = game.data.guesses[1];
            let player1Right = player1Guesses[player1Guesses.length - 1].word === game.data.answers[0];
